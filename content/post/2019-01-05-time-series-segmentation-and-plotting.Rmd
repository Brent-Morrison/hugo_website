...
This post is going to analyse a component of the data downloaded [here](https://brentmorrison.netlify.com/post/financial-data-aggregator/), plotting the slope of the yield curve, calculated as the spread between the federal funds rate and 10 year treasury note rate, and the S&P 500 index.  We will also discretise the yield curve into bins or categories based on its level and change, plotting a histogram of subsequent S&P 500 returns for each bin.  

### Background  
The yield curve has been theorised to be a powerful predictor of the business cycle,  [(wikipedia reference)](https://en.wikipedia.org/wiki/Yield_curve#Relationship_to_the_business_cycle).  We are not interested in the business cycle, but rather if the yield curves is a leading indicator to the stock market.  Specifically, does any particular level, change (or combination thereof), of the yield curve provide information about future stock market returns.  For example if the yield curve is in the lowest third of its historical range, and is increasing, do subsequent stock market returns differ compared to other periods?   

The logic for using a discretisation approach, as opposed to interpreting regression co-efficients is motivated by the expectation of non-linearities in the relationship between the yield curve and stock market.  If the inversion of the yield curve is considered a signal of lower future economic activity as theorised above, then it would be expected that the status of the yield curve at other times is not relevent.  Although economic activity does not necessarily correlate to stock market returns, this non-linearity is the initial expectation for any possible relationship between the yield curve and stock market. 
...
```
Next, we calculate the yield curve subtracting the fed funds rate from the 10 year treasury note rate, select the required data set and categorise the yield curve into bins representing specific level and change values.  The code block below decomposes the time series of the yield curve into 6 bins. The level of the yield curve is split into terciles or three bins containing an equal number of months and the change in level is categorised as either an increase or decrease (positive or negative 6 month change).  The combination of these is then lagged 6 months and 12 months resulting in 18 bins in total.
...
str(x2.1)
```
Three values are returned, the current and two lagged values.  Note the format of these values, this will be used later in the the histogram. 

In order to enable segmentation for the histogram we need to create dummy variables for each of the 18 bins. 
...
Recall that we are interested in the predictive power of yield curves to the stock market.  Should there be a relationship between the status of the yield curve defined by our level / change bins and future returns, returns subsequent to when specific bins are triggered will differ to all other returns.  
...
If we look at the plot of the yield curve we can confirm this visually.  It does appear for example that the drawdowns in years 2000 and 2008 are preceded preceded by anincrease in the yield curve from a low level.   The second most significant bin for the current value of the yield curve is when it is in the mid tercile and decreasing, subsequent returns are 0.75% higher with a K-S p value of 3.7%.  For reference, average monthly returns over the time under analysis are  `r  percent(round(mean(econ_fin_data$rtn_m[-length(econ_fin_data$rtn_m)][-1]),4))`.  

...
This post is going to analyse a component the data downloaded [here](https://brentmorrison.netlify.com/post/financial-data-aggregator/), plotting the spread between the Federal funds rate and 10 year treasury rate (the yield curve) against the S&P 500 index.  We will also decompose the yield curve into bins or categories based on its level and change, plotting a histogram of subsequent S&P 500 returns for each bin.  

### Background  
The yield curve has been theorised to be a powerful predictor of the business cycle,  [(wikipedia reference)](https://en.wikipedia.org/wiki/Yield_curve#Relationship_to_the_business_cycle).  We are not interested in the business cycle, but rather the yield curves predictive relationship to the stock market.  Specifically, does any particular level, change (or combination thereof) of the yield curve provide information about future stock market returns.  For example if the yield curve is in the lowest third of its historical range, and is increasing, do subsequent returns differ compared to other periods.   
...
```
Extract the current status of the yield curve with respect to the level and change bins just created.  This will call out the current status of each of the three level / change bins. 
...
x8 %>% filter(date == "2018-11-01") %>% select(-rowIndex, -Value) %>% 
  kable() %>%  kable_styling()
```
Note the same bins labelled "In" as that returned above.  
...
The code below creates a nested data frame so that a K-S test can be performed for each bin, assessing the dissimilarity of subsequent monthly returns when that bin is triggered and when not. The difference in average return is also calculated. These measurements will be used to narrate the plot. 
...
Lastly, the code below plots the S&P 500 and yield curve overlaying periods when the maximum drawdown is greater than 20% or the return is less than 2.5% for the next 6 monthly period.
...
### Limitations
The binning approach is quiet crude in that by design it peaks into the future.  The measurement of the tercile barriers during the 1970's uses data from the 2000's for example.  A more robust approach would be to utilise a rolling fixed starting point window to estimate terciles for binning.

...
Moving on, we need to join are cluster assessment back to the oringinal monthly dataset.  The code block below joins the cluster labels to the monthly data set and then fills forward the cluster membership.
...
We expect to see an upward sloping line as the P (price or market capitalisation) in the logPB increases with greater ROE (return on equity).  This is primarily what has been observed.  A couple of clusters do have an inverted relationship and a very low number of individual observations.    

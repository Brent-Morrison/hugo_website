...
```{r pb_roe_check, error=FALSE, message=FALSE, warning=FALSE, echo=TRUE}  
...
labs <- simfin.m %>% filter(!is.na(clust.name), me.date == "2017-12-31") %>% 
  group_by(clust.name) %>%
  filter(resid == max(resid) | resid == min(resid)) %>% 
  select(Ticker, me.date, clust.name, logPB, ROE, resid)
...
xx1 <- simfin.m %>% 
  ...
  select(Ticker, mkt.cap, TotalEquity, BooktoMarket, 
         TotalDebt, logPB, ROE, resid, vol_3m, 

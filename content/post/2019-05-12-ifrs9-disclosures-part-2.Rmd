...
```{r step_1_21, error=FALSE, message=FALSE, warning=FALSE, echo=TRUE, fig.width=8.25, fig.height=8.25}
# View summary statistics
...
```{r step_3, error=FALSE, message=FALSE, warning=FALSE, echo=TRUE, fig.width=8.25, fig.height=8.25}  
set.seed(123)
...
The code block below creates the required inputs to the model, fits the model to each cluster and date, and then joins this data back to our main dataframe `simfin.m`.  This block of code relies heavily on the nesting functionailty in the `purrr` package.
...
simfin.m <- simfin.m %>% left_join(simfin.pbroe) %>% 
  ...
  # Add percent rank of residual  TO DO - add +ve or -ve residual flag to indicate if
...
*Determine Why cluster `oa.ta_nca.ta_td.ta_da.ta`as at 31 December 2017 returning only one stock despite the cluster size being 81?*  
 
#### Step 5 - Apply the Merton distance to default model and rank stocks by probability of default

TO DO - insert introductory explanation of Metorn model here.  Why does full version need iteration?  Show summarised version.  What does the DD actually represent?

The Merton distance to default model [Merton for dummies](https://econpapers.repec.org/paper/utsrpaper/112.htm)  
...
# Create Merton DD.  TO DO - resolve infinity
simfin.m <- simfin.m %>% 
...
TO DO - visualise the results of the DD.
...
xx1 <- simfin.m %>% 
  filter(me.date == "2017-12-31"
         #,clust.name == "oa.ta_rev.vol_da.ta_rev.ta"
         ) %>% 
  select(Ticker, mkt.cap, TotalEquity, BooktoMarket, 
         TotalDebt, logPB, ROE, PBROE.resid, vol_3m, 
         PBROE.rank, MertonDD, MertonDD.rank,
         avg.rank, comb.rank, cover, ECL)


xx2 <- simfin.pbroe1 %>% 
  unnest(tidied) %>% 
  filter(me.date == "2017-12-31", clust.name == "oa.ta_rev.vol_da.ta_rev.ta")
```

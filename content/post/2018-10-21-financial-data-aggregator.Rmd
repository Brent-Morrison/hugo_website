...
This post is going to document an R script that will download and aggregate various financial and economic time series.  This data will be used in future posts to perform various modelling and analysis work.  
...
str(econ.m)
```
It is apparent with the NA's returned that not all times series begin at the specified start date.  Everything requested is there however.  Next we want to download and join the S&P 500 index time series.
...
The attribute y1 is a binary indicator that looks forward over the next 6 months and return 1 if the maximum draw down is greater than 20% or the return is less than 2.5% (these parameters can be changed as detailed above).  This will be the independent or forecast variable for future analysis.

...
[^2]: For the un-initiated, fundamental data refers to accounting balances recorded in a company's financial statements (the balance sheet, income statement and cash-flows). Price data of course is the traded share price.  
...
This post is a little light on visualisation and code.  I'll finish up with a fancy plot of the cross sectional skewness effect for US stocks.  This is taking data directly from the database.
...
sql2 <- "select date_stamp, symbol, skew_ari_120d_dcl, close from access_layer.return_attributes"
...
#  filter(fin_nonfin == 'non_financial') %>% 
#  group_by(date_stamp) %>% 
#  slice_min(mkt_cap, n = 100) %>% 
#  summarise(
#    assets = sum(total_assets),
#    cash = sum(cash_equiv_st_invest),
#    lt_debt = sum(lt_debt),
#    cash_ratio = cash/assets,
#    debt_ratio = lt_debt/assets
#  )
...
price <- df_price_raw %>% 
  ...
  group_by(date_stamp) %>% 
  mutate(fwd_rtn_1m = scale(fwd_rtn_1m, center = TRUE, scale = FALSE)) %>% 
  group_by(date_stamp, skew_ari_120d_qntl) %>% 
  summarise(fwd_rtn_1m = mean(fwd_rtn_1m, na.rm = TRUE))

gga <- ggplot(
  filter(price, date_stamp == as.Date('2019-01-31')),
  #filter(price, date_stamp >= as.Date('2019-01-31')), 
  aes(skew_ari_120d_qntl, fwd_rtn_1m)
  ) +
  geom_col(aes(group = date_stamp), linetype = 1) +
  xlab('') +
  ylab('') +
  scale_y_continuous(
